...
const AdminPanel = () => {
  ...
  const handleAddOrUpdateProperty = async () => {
    try {
      if (editMode) {
        await axios.put(
          `${API_BASE}/admin/property/${editingPropertyId}`,
          form,
        );
      } else {
  ...
  const saveStatus = async (userId, propertyId) => {
    ...
    try {
      await axios.patch(`${API_BASE}/activity/status`, {
        userId,
        propertyId,
        status,
      });
...
const handleCheckboxChange = (value) => {
  const currentTypes = form.generalInfo.propertyType;
  const newTypes = currentTypes.includes(value)
    ? currentTypes.filter((t) => t !== value) // Remove if already there
    : [...currentTypes, value]; // Add if not there

  setForm({
    ...form,
    generalInfo: { ...form.generalInfo, propertyType: newTypes },
  });
};
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            <h4>Features</h4>
            <input
              placeholder="Interior Features (comma-separated)"
              value={form.features.interior.join(",")}
              onChange={(e) =>
                setForm({
                  ...form,
                  features: {
            ...
            />
            <input
              placeholder="Exterior Features (comma-separated)"
              value={form.features.exterior.join(",")}
              onChange={(e) =>
                setForm({
                  ...form,
                  features: {
            ...
            />
            <select
              value={form.features.basement}
              onChange={(e) =>
                setForm({
                  ...form,
                  features: { ...form.features, basement: e.target.value },
                })
              }
            >
            ...
            <input
              ...
              value={form.generalInfo.propertyAddress}
              onChange={(e) =>
                setForm({
                  ...form,
                  generalInfo: {
            ...
            <div>
  <p>Select Nearby Landmarks:</p>
  {propertyOptions.map((opt) => (
    <FormControlLabel
      key={opt.value}
      control={
        <Checkbox
          checked={form.generalInfo.propertyType.includes(opt.value)}
    ...
    />
            ...
            <input
              ...
              value={form.generalInfo.yearBuilt}
              onChange={(e) =>
                setForm({
                  ...form,
                  generalInfo: {
            ...
            <input
              ...
              value={form.generalInfo.squareFootage}
              onChange={(e) =>
                setForm({
                  ...form,
                  generalInfo: {
            ...
            <input
              ...
              value={form.generalInfo.numberOfBedrooms}
              onChange={(e) =>
                setForm({
                  ...form,
                  generalInfo: {
                    ...form.generalInfo,
                    numberOfBedrooms: parseInt(e.target.value),
                  },
                })
              }
            />
            <input
              ...
              value={form.generalInfo.numberOfBathrooms}
              onChange={(e) =>
                setForm({
                  ...form,
                  generalInfo: {
            ...
            <input
              type="number"
              placeholder="Floors"
              value={form.generalInfo.numberOfFloors}
              onChange={(e) =>
                setForm({
                  ...form,
                  generalInfo: {
            ...
            <input
              ...
              value={form.radius}
              onChange={(e) =>
                setForm({ ...form, radius: parseInt(e.target.value) || 0 })
              }
            />
            ...
            <button onClick={handleAddOrUpdateProperty}>
              {editMode ? "Update" : "Submit"} Property
            </button>
            <button
              onClick={() => {
                setShowModal(false);
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 20 }}>
        {properties.map((prop) => (
          ...
          >
            ...
            <p>
              <strong>Radius:</strong> {prop.radius || 0}m
            </p>
